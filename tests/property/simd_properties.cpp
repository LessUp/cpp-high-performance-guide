...
RC_GTEST_PROP(SIMDWrapperProperties, AddArraysCorrectness, ()) {
    ...
    auto gen_float = rc::gen::map(rc::gen::inRange(-1000, 1000),
                                  [](int x) { return static_cast<float>(x) * 0.1f; });
...
RC_GTEST_PROP(SIMDWrapperProperties, DotProductCorrectness, ()) {
    ...
    auto gen_float = rc::gen::map(rc::gen::inRange(-100, 100),
                                  [](int x) { return static_cast<float>(x) * 0.01f; });
    ...
    RC_ASSERT(std::fabs(simd_result - ref_result) <=
              tolerance * std::max(1.0f, std::fabs(ref_result)));
}
...
RC_GTEST_PROP(SIMDWrapperProperties, ScaleArrayCorrectness, ()) {
    const auto size = *rc::gen::inRange<size_t>(4, 10000);
    const float scalar = *rc::gen::map(rc::gen::inRange(-100, 100),
                                       [](int x) { return static_cast<float>(x) * 0.1f; });
...
RC_GTEST_PROP(SIMDWrapperProperties, ClampArrayCorrectness, ()) {
    ...
    auto gen_float =
        rc::gen::map(rc::gen::inRange(-200, 200), [](int x) { return static_cast<float>(x); });
...
TEST(SIMDSpeedupTests, VectorizedFasterThanScalar) {
    ...
    // SIMD implementation
    auto simd_add = [&]() { hpc::simd::add_arrays_wrapped(a.data(), b.data(), c.data(), N); };
    ...
    std::cout << "SIMD Level: " << hpc::simd::simd_level_name(hpc::simd::detect_simd_level())
              << std::endl;
