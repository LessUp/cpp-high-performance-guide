...
- **可运行优先**：每个阶段结束必须可构建、可运行、可验证。
- **最小可复现**：所有性能结论必须能通过脚本或明确命令复现。
- **文档同步**：示例代码变更必须同步更新模块 README 与学习路径。
- **向后兼容**：新增示例不破坏现有 CMake 模板与基准测试框架。
...
- 平台：Linux 为主要验证平台，macOS/Windows 为尽力支持。
...
- **性能波动**：提供多次重复与统计指标，避免偶然数据。
- **平台差异**：在文档中标记平台/编译器差异与已知限制。
- **工具缺失**：在文档中提供降级方案或替代步骤。
- **复杂度增长**：保持模块边界清晰，禁止跨模块隐式依赖。
