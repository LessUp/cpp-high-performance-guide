...
| **03. Modern C++** | `examples/03-modern-cpp/`, `buffer.hpp`, `compile_time.hpp`, `ranges_utils.hpp`, `vector_reserve.hpp` | `include/hpc/core.hpp` | `tests/unit/modern_cpp/`, targeted benchmarks | [Best Practices](/en/guides/best-practices) |
| **04. SIMD vectorization** | `examples/04-simd-vectorization/`, `simd_utils.hpp`, `simd_wrapper.hpp` | CPU-feature dispatch examples | `tests/unit/simd/`, `tests/property/simd_properties.cpp`, `simd_bench` | [Optimization Decision Tree](/en/guides/optimization-decision-tree) |
| **05. Concurrency** | `examples/05-concurrency/`, `concurrency_utils.hpp`, `lock_free_queue.hpp` | OpenMP-enabled examples | `tests/unit/concurrency/`, `tests/property/concurrency_properties.cpp`, `tsan` | [Validation Doctrine](/en/academy/validation-doctrine) |
